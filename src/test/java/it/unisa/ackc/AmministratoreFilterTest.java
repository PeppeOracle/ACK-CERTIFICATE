...
public class AmministratoreFilterTest {
    @Mock
    ...
    public void test02() throws IOException, ServletException {
        AmministratoreFilter filter = new AmministratoreFilter();
        ...
        filter.doFilter(request, response, chain);
        verify(response).sendRedirect(request.getContextPath() + AmministratoreFilter.PERMESSO_NEGATO_JSP);
    }
    ...
    public void test04() throws IOException, ServletException {
        ...
        when(request.getSession(any(Boolean.TYPE))).thenReturn(session);
        filter.doFilter(request, response, chain);
        verify(response, never()).sendRedirect(request.getContextPath() + AmministratoreFilter.PERMESSO_NEGATO_JSP);
    }
}
